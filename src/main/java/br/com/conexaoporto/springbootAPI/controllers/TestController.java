...
public class TestController {// Esse controller contêm exemplos relacionados a area de test do site
	...
	// EXEMPLO CADASTRO
	@PostMapping("/testCadastroProfissional") // <- anotação para tratamento de metodos POST
	private String novoProfissional(@Valid Profissional profissional, BindingResult result, Model model) { // BidingResult
																											// para
																											// validação
																											// de dados,
																											// Model é
																											...
																											// thymeleaf
		if (profissionalRepo.findByEmail(profissional.getEmail()) != null) {
			result.addError(new ObjectError("profissional.email", "email já cadastrado"));// TODO: descobrir como
																							// retornar esse erro para o
																							// thymeleaf e exibir na
																							// pagina
			// System.out.println(result.hasFieldErrors("profissional.email"));
		}
	...
	// EXEMPLO LISTAR DADOS (especifico)
	@PostMapping("/testPerfil")
	public String exibirDadosProfissional(@RequestParam(name = "email") String email, Model model) {// @RequestParam
																									// associa a
																									// variavel da barra
																									// de endereços ao
																									// parametro
	...
	@PostMapping("/testpreloadAtualizarDados") // Isso é um 'intermediario' para carregar os dados do usuario e retornar
												// eles para a pagina certa
	public String atualizarDadosProfissional2(@RequestParam(name = "codUsuario") long id, Model model) {

		model.addAttribute("usuario", profissionalRepo.findById(id));// Encontra o usuario baseado no id e salva as
																		// informações dele no model
		return "test/atualizar-profissional";
	}
	...
	public String atualizarDadosProfissional(@RequestParam(name = "codUsuario") long id,
		...
		Profissional profissional = profissionalRepo.findById(id); // Encontra os dados do ususario usando o ID
		// Atualiza os valores do objeto com os mais recente, se o valor informado
		// estiver em branco ele mandem o valor anterior
		profissional.setNome(!nome.isEmpty() ? nome : profissional.getNome());
		profissional.setSenha(!senha.isEmpty() ? senha : profissional.getSenha());
		profissional.setTelefone(!telefone.isEmpty() ? telefone : profissional.getTelefone());
		profissional.setDescricao(!descricao.isEmpty() ? descricao : profissional.getDescricao());
		profissional
				.setAreaDeInteresse(!areaDeInteresse.isEmpty() ? areaDeInteresse : profissional.getAreaDeInteresse());
		profissional.setOcupacao(!ocupacao.isEmpty() ? ocupacao : profissional.getOcupacao());
		profissional.setNivelDeEscolaridade(
				!nivelDeEscolaridade.isEmpty() ? nivelDeEscolaridade : profissional.getNivelDeEscolaridade());
	...
	public String loginProfissional(@RequestParam(name = "email") String email,
		...
		Usuario usuario = profissionalRepo.findByEmail(email) != null ? (Usuario) profissionalRepo.findByEmail(email)
				: null;
		autenticacaoHelper autenticador = new autenticacaoHelper();// essa classe armazena informações para transmitir
																	// tudo de uma vez pelo model
		if (usuario != null) {
...
>>>>>>> branch 'master' of https://github.com/c-ferraz/residenciaPD_backend.git
